...
Finally, let's set up the tests: just like yesterday, you'll find a `step1_tests.zip` file in your repository with a `tests` folder inside it.\
...
When hitting the endpoint, it must respond `world` with the status `200`.
...
Finally, let's create an endpoint `/jwt/me` to retrieve user data on method `GET`.
...
<details>
  ...
  Yesterday you discovered how to manipulate a database with Ent. Today, you've built an API with Gin.
